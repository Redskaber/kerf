...
//!
//! **v6.1 扩展（next4 第八轮「2026 接口预留完整性审查」，r12 批次）**：
//! - [`toolchain`]：P0 三项（LSP/IDE 查询 `LanguageService` + `IncrementalAst`
//!   / 调试信息 `DebugInfoGenerator` + `DebugTraceable` / 增量编译查询
//!   `QuerySystem` + `Query`）+ P1 `CompilerService`/`Serializable` +
//!   P2 `PackageManager`/`ExternalModule`/`AiAssistant`；
//! - [`ffi`]：P1 FFI 边界（`ExternalType`/`FfiCall`/`FfiBoundary`——GC 隔离
//!   pin/unpin 协议；实现 Stage 2，不入自举链）；
//! - [`codegen`]：P1 多目标后端（`CodegenBackend`/`WasmBackend`——LLVM
//!   永不进入自举链，§21 后端策略）。
//!
...
//! 全部预留以 Probe 实现体编译通过证明契约冻结（测试锚点：
//! `reserved_signatures_are_frozen` 系）。
