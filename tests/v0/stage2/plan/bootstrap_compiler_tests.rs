...
fn parity_const_pool_dedup() {
    // 常量池去重（§19.3 陷阱：(do 1 1 1) 不膨胀——关联列表首插序镜像）
    parity("(do 1 1 1)");
    parity("(do \"a\" \"a\" 1 \"a\")");
...
fn parity_lambda_basic() {
    ...
    parity("(fn () 1)");
    parity("(fn (a b c) (list a b c))");
}

#[test]
fn parity_lambda_nested_captures() {
    // 嵌套闭包捕获（设计 §4 S1 特化 case）
    parity("(define (f x) (fn (y) (x y))) (f 3)");
    ...
    // 同层双捕获（捕获序 = 自由变量首现序）
    parity("(define (f x y) (fn () (list x y))) (f 1 2)");
    // 形参遮蔽（shadow-pop 口径逐位镜像）
    parity("((fn (x) ((fn (x) x) 3)) 4)");
    // 捕获与全局引用并存（Global 零捕获 + free_vars 并集口径）
    parity("(define (f x) (fn (y) (+ x g))) (define g 1) (f 2)");
}
...
fn parity_lambda_set_bang_paths() {
    ...
    // 全局 assign
    parity("(define x 1) (assign x 9) x");
    // assign 值先入栈 DUP（栈平衡不变式 1）
...
fn parity_tail_position_threading() {
    ...
    // do 末项继承尾位（do 尾调用 = 尾调用——scheme 语义）
    parity("(define (f n) (do 1 (f (- n 1))))");
    // 非尾位（体中表达式位置 → Call）
...
fn parity_err_define_position() {
    ...
    parity_err("(define (f x) (do (define y 2) x))");
    parity_err("(fn (x) (do 1 (define y 2)))");
    parity_err("(fn (x) (if true (define y 2) x))");
    parity_err("(define (f x) (let ((y 1)) (do (define z 2) y)))");
}
...
fn parity_sugar_let_family() {
    ...
    // let 体多形式（脱糖为 do 语义）+ 尾位穿线
    parity("(let ((n 0)) (assign n (+ n 1)) n)");
...
fn parity_sugar_and_or_while() {
    ...
    parity("(define i 0) (while (< i 5) (assign i (+ i 1))) i");
    parity(
        "(define (count-up n)
           (let ((i 0))
             (while (< i n) (assign i (+ i 1)))
             i))
         (count-up 3)",
    );
}
...
fn parity_macros_corpus() {
    ...
    // swap!（let+assign 混合）/ my-or（递归宏 + 省略号）
    parity(
        "(define-syntax swap!
           (syntax-rules ()
             ((swap! x y)
    ...
    parity(
        "(define-syntax def-twice
           (syntax-rules ()
             ((def-twice n v) (do (define n v) (define n2 v)))))
         (def-twice x 7) x",
    );
}
...
fn behavior_closures() {
    ...
    behavior("(define (make-adder n) (fn (x) (+ x n))) (define add5 (make-adder 5)) (add5 37)");
}
...
fn behavior_higher_order() {
    ...
    behavior("(define (map f lst) (if (is-nil lst) nil (cons (f (head lst)) (map f (tail lst))))) (map (fn (x) (+ x 1)) '(1 2 3))");
}
