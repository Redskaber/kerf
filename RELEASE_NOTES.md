## v0.11.0-r44（2026-09-17）——E5 S3 M-R 名面臂：内部关键字全栈一致性深审（D34-D40）+ 五面同词根 + 自举桥 tag 域重构（65-a/65-b/65-z/65-web——用户指令「内部关键字和候选衍生关键字的完全重新筛选 + 原语本身也是关键字/暴露面与编译器内部一致性[不然内部设计一套开发一套算什么？] + 关键字正交性 + 自举桥同类问题也需重构[正确 > 妥协]」+「按 sop.md 继续推进」+「同步完整打包 tar.gz 与 web page」）

### 交付零：内部关键字全栈一致性深审（65-a——22 §13 D34-D40 + 四查询实证）
- **四套名面现状矩阵**（22 §13.1）：表面关键字 / 自举桥 quote 符号与 tag / Rust ADT 变体名 / kind_name 诊断名——**四语义 8 处不一致 + Keyword 枚举内自身不一致 3 处**（fn↔'lambda↔Lambda↔"lambda" / assign↔'set[第三形——非 set! 非 assign]↔SetBang↔"set!" / do↔'begin↔Begin↔"begin" + var/lit/app 两形并存）；r43「三域零变更」被裁定为**把债务当资产引用的过度引申**（域隔离是防误伤流程 ≠ 名面漂移合法）
- **一致性总则（D34——判据新增）**：「一个语义一个名」：原语级关键字的表面名 = 桥 quote/tag = ADT 变体词根 = kind_name = 渲染面，五面同词根；派生关键字经宏/糖展开到原语（无独立 ADT 节点）；无表面关键字的语义（apply/var/literal）用内部统一名三面同名。**原则 31 精确化**：分离的是语法形态（皮肤可替换），不是名词根（名面一致）——名面漂移不是分离原则的推论而是其违反（正向：Python AST 同词根[ast.If↔if]；反向：Racket `#%plain-lambda` 双轨家族病活体实证）
- **M-R 重命名族逐名裁定（D35——修正 01 §8.3 原候选）**：五件 GO（`Lambda→Fn`/`SetBang→Assign`/`Begin→Do`/`App→Apply`/`VarRef→Var`——M-R 与 M-E 解耦：D5 否决的是效应化实现，纯重命名零语义载荷独立合法）；**两件 NO-GO**（`If→Branch`：表面 if 是 D22 跨家族通用语，重命名制造新分离——一致性 > 行为导向微优化；`Literal→Const`：literal 精确覆盖 quote 产物，const 偏编译期常量概念，不可变性由类型系统保证[原则 23/30]名无需编码）——判据先于先例的操作实例（01 §8.3 是 next3 轮裁定，新判据推翻其中两行）
- **S3 分臂裁定（D39）**：r44 = M-R 名面臂 + 三域一致化（~270 Rust 处 + 桥语料 ~30 处 + 测试期望）；结构臂（Define 脱糖/Do→Let 链/Module 迁移/de Bruijn IR）归 r45+ 分臂承载（每臂独立 commit + 字节码 parity 断言先行）；**六域候选空间完备性收敛证明（D40）**：Fn/Assign/Do/Apply/Var/Literal 六域 × 4-5 候选全展开，每域收敛唯一解 + 否决记录保留（Branch/Const 两否决入档）
- **关键字正交性对账（§13.3——用户指令「每个关键字的定位/职责/边界/能力」）**：9 原语级关键字 × 五要素表 + 3 内部名 + 13 派生/声明/数据位——25 表面名每名职责唯一无重叠（M-R 零语义载荷不改正交结构）；知识搜索四查询实证（tool-results/r44-search/：Python AST 节点同词根 / 自举双实现同步 / Racket #%plain-lambda 双轨 / PEP8-C# 命名一致性）

### 交付一：M-R 名面臂实施（65-b——§3.2 六命令 + 834:0:0）
- **变体五件重命名**（expr.rs 枚举定义 + OCaml 规范块同词根化[规范↔代码互锚] + 15 crate 文件 + 7 测试文件 ~244 处 + 注释面全清扫——crates/tests/examples 旧名 grep 零残留）；**Keyword 枚举对齐**（D36：`Keyword::Fn/Assign/Do` 变体名 = as_str 字面 + `Keyword::ALL` 25 名单源[预内部化与 roundtrip 共享]）；**kind_name 12 名表**（D37：fn/apply/if/var/literal/assign/define/do/module/require/perform/handle——诊断面与桥 tag 同名穿透）
- **自举桥 tag 域重构（D38——用户指令点名载荷）**：六处改名 `'lambda→'fn`/`'set→'assign`/`'begin→'do`/`'app→'apply`/`'lit→'literal`（全词统一——两形并存清偿）/`'var` 维持；expander.krf（产出点）+ compiler.krf（双 pass 匹配点）+ bootstrap_compiler.rs（tag 表）+ bootstrap_expander.rs（消费臂）四文件双侧对称迁移；tag 改名 parity 影响分析：tag 是数据协议标记非 bytecode 组成——门 A CoreExpr parity 恒成立 + 门 B fixpoint 随语料变更重建（SHA 重锚合法）
- **新增机器断言 ×2**：`s3mr_kind_name_keyword_face_penetration`（五面穿透——9 原语级 kind_name ∈ Keyword 注册面 + 3 内部名 ∉ 关键字面[内部名独立域不侵蚀 E0020 全域排他]）+ `keyword_all_roundtrip_and_distinct`（25 名全量 roundtrip + 字面互异 + 计数锚）；architecture_audit 12 名表同步 + r43 遗留 unused import 清偿（clippy -D warnings 0）
- §3.2 六命令 clean 起步全绿（build 14.26s 零告警 / check 0 / fmt 0 / clippy --all-targets -D warnings 0 / **test 834:0:0**[615 集成 + 219 单元——净 +2]）+ 五审计集 EXIT 0 ×5（233）+ CLI 四路径（fib 144 / macros 42 / check ok / E0021 端到端 REAL_EXIT 1 携「现代形式「fn」」）
- 对账：22 v1.7（§13 D34-D40 + 13.4 实施实录）/ 01 v6.3（§2 OCaml 块同词根化 + §8.3 修正版 + §8.5 五面锚）/ 17 v6.6（原则 31 精确化）/ 20 v1.5 注记（S3-M-R 行 ✅）/ e5-plan v1.2（S3 分臂记录）/ matrix v0.1.0-r44 / RELEASE v0.11.0-r44 / v0.5-roadmap v0.8.0-r44

- **里程碑：E5 S3 M-R 名面臂交付——「1970 表面」清算第三腿（S1 库面 r42 + S2 表面 r43 + S3 内部名面 r44 三腿闭合，五面同词根终态）；下一步 E5 S3 结构臂 r45+（Define 脱糖/Do→Let 链/Module 迁移/de Bruijn IR——字节码 parity 最严口径）→ S4 收口（非正交点 3→≤1 + 门审 ≥30 case）**
...
- **E5 窗合法开启**（23 §2.2 红线 3「禁止无信号开窗」合规）：①窗 M 后 v0.6 稳定期（M3 闭环 837:0:0 + 弃用期跨版本核对[W1001 落地 v0.7 → v0.8 一稳定版全绿 → v0.9 移除]）②12 §2.5.1 全行终态（磁盘实况逐行核对）③外循环投票 5/5 = 100% GO（e5-plan §6 记录）④自举 parity 链保护方案评审通过
- **设计载体**：docs/develop/v0/stage-3/e5-removal-round-plan.md v1.0——三腿分步（S1 表面腿[本轮 ✅] / S2 关键字腿[lambda→fn 等 N4 面] / S3 ADT 腿[Define 脱糖/Begin→Let/de Bruijn] + S4 收口）；parity 断言矩阵复用 i1 门 A/B/C 三门（零新造——原则 6）；`?`/`->` 词法域收紧裁定 = 维持不收紧（否决记录在位——重评条件 = 包生态名冲突实证）

### 交付一：移除轮 S1 表面腿（63-b——§3.2 六命令 + 828:0:0）
- **旧名 27 件退役**：`BUILTIN_ALIASES` 双注册机制下线（20 §7 移除轮行兑现）——主注册 27 对旧名→现代名迁移（`REMOVED_BUILTIN_NAMES` 退役表单源：旧名→现代名指引映射）；注册面 84→57 扁平（+47 限定 = 104 全授权 / 92 零授权）；BUILTIN_SIGS 旧条目删除（83→56 维持）；STDLIB_MODULES 底层 4 名现代化
- **E0021 已移除旧名错误**：driver `verify_qualified_refs` VarRef 臂扩展（与 E0014 同 traversal——遮蔽/接管豁免同口径：N3 局部绑定胜出 + 用户接管合法[退役的是内置注册面非符号宇宙层]）；诊断携现代名指引（比裸 E0004 未绑定 actionable——迁移期 DX 最优）；CLI 端到端 REAL_EXIT 1
- **W1001 退役 + W1003 落地**：弃用族下线（23 §3.4 生命周期四阶段完整走完：引入 v0.5 → 默认 v0.6 → 弃用 v0.7 → 移除 v0.9）；W1003 宏名遮蔽警告（22 §11 D11 排期承诺清偿——Stx 层 `collect_macro_names` 唯一数据源 + `builtin_name_exists`[57 扁平 + 47 限定] + 宏胜出行为不变）
- **全语料现代名终态**：引导 .krf ×4（compiler 249 + expander 511 + reader 200 + preamble 17 处替换）+ 21 测试文件 ~350 处 + examples——词法边界替换（grep 零旧名实证）；**迁移实录：compiler.krf `tail` 参数名与现代内置名碰撞 → `tail-pos` 重命名**（词法遮蔽合法但迁移期文本碰撞——如实登记）
- 负例组 27 名全覆盖（stdlib_tests removed_* 17 case：E0021 三断言 + 参数遮蔽/用户接管合法豁免 + 闭合守卫双向对账）+ namespace_tests s1 组 4 case（W1003 正例/零误报/限定名遮蔽 + E0021 恢复健康/位置面）；hm.rs 结构重解释臂 head/tail 现代化 + anf.rs eq；M3 门审 P07 计数锚更新（43:0 APPROVED 维持）
- §3.2 六命令 clean 起步全绿（build 15.20s 零告警/check 0/fmt 0/clippy 超集 0/**828:0:0**[837 - 9——移除轮测试面合法收缩：28 parity case 收敛为 17 退役负例 + 2 W1001 case 演进为 4 W1003/E0021 case]）+ 五审计集 EXIT 0 ×5（233）+ CLI 四路径 + E0021/W1003 端到端
- 对账八面 + 文档回写：20 v1.5 / 22 v1.5 / 23 v1.5 / 18 v6.7（E0021 + W1003 码位落位）/ 12 / 09 v6.11 / **TD-027 清偿关闭（三腿全兑现：批次 L r38 + M r39-40 + 移除 S1 r42）** / matrix v0.1.0-r42 / pipeline r42 / v0.5-roadmap v0.6.0

- **里程碑：E5 窗开启（四信号全过）+ S1 表面腿交付——1970 表面清算首腿（库表面现代名唯一注册面）；下一步 E5 S2 关键字腿（lambda→fn/set!→set/begin→seq——01 §7.2 候选 + 全语料 + 负例组）→ S3 ADT 腿 → S4 收口（非正交点 3→≤1）**

## v0.8.0-r41（2026-09-15）——批次 M 收口 M3 + 语言形式深审轮：E0020 保留字绑定禁令 + M3 门审 APPROVED（62-a/62-b/62-z/62-web——用户指令「内部语法设计/通解特解/词法语法语义无二义性/层级组织/(a b c) 通解性/多模型组织 + 2026 前沿 + 内循环迭代收敛」+「按 sop.md 继续推进」+「同步打包 tar.gz + web page」）

- **深审 D10（P1 文档-代码冲突当场修复）**：22 §2.1 N4 不变量「不是值、不可引用、不可遮蔽」实测被违反（`(define if 5)` 合法 + 值位可读 + 操作位恒关键字 = 用户绑定**静默失效**——Lisp 家族双轨陷阱）→ **E0020 严格保留字绑定禁令**（25 关键字全域禁作绑定名：define/set!/lambda 参数/let 系/define-syntax 宏名/module 名/import as 别名/handle 双绑定器；Stx 源码面 + CoreExpr 展开产物面双层防御；syntax-rules 模板数据域跳过[零误报]；quote 位 `'if` 数据符号豁免[同像性维持]；值位引用恒未绑定 fail-closed）——12 case 锚定 + 837:0:0 净 +12 零回归
- **深审 D11-D18 七项裁定**（22 §11——六查询实证 + Rhombus v1.0 2026-06 发布深读）：D11 W1003 宏遮蔽警告排期移除轮 / D12 `/` vs `::` 维持（三重依据：模块不嵌套 + 域成员语义读法 + 除法二义性已分立——`.`/`::` 符号期权保留）/ D13 `(a b c)` 前缀 = 当前工程最优解（UFCS 更强形态 + Rhombus enforestation 代价 ×10 对照 + 终态多语法窗维持——「kerf 不被家族限制」三件兑现）/ D14 负数字面量显式无二义 / D15 N4 三件套全编译期执行 / D16 functor 不引入（OCaml 蔓延痛点反向印证）/ D17 维持括号（缩进敏感批判）/ D18 **词法语法语义二义性面全清**（六组潜在歧义源逐一显式裁定）
- **M3 收口门审 APPROVED**：stage3_gate_audit_m3 **43 case**（A11/B9/C6/D5/E5/P7——七类全覆盖 + M1/M2/D10 三修复边界 ≥5 + E2E 全导入路径三通道[限定直引/非限定注入 D7 过渡期语义如实审计/别名归一] + io 授权第四通道 + 计数锚）——**窗 M 出口条件达成**（R-N1~N8 全 case + B1-B3 契约 + 组合闭包 + E2E——23 §2.2）
- §3.2 六命令 clean 起步全绿（build 14.41s 零告警/check 0/fmt 0/clippy 超集 0/837:0:0）+ **五审计集 EXIT 0 ×5**（190+43=233）+ CLI 四路径（fib 144/macros 42/effect_stress 120/check ok+E0005 REAL_EXIT 1）+ E0020 CLI 端到端 REAL_EXIT 1
- 对账八面 + 文档回写：22 v1.4（§11 D10-D18 + §2.1 N4 三件套执行注记）/ 18 v6.6（E0020 码位落位）/ 02 v6.3（§8 Rhombus v1.0 对表注记）/ 14 r41 前沿更新（Rhombus v1.0 三重启示）/ matrix v0.1.0-r41 / pipeline v0.6.0-r41 / 12 v6.13 / 23 v1.4 / 09 v6.10 / v0.5-roadmap v0.5.0
- **里程碑：批次 M 全链闭环（M1 r39 + M2 r40 + 深审 D10 r41 + M3 门审 APPROVED）——窗 M 出口条件达成；下一步移除轮（与 E5 同窗，23 §2.2 触发表驱动）**
...
- **七模块 47 限定名注册**：`builtins.rs STDLIB_MODULES` 单源静态表（20 §5.2 模块树转译：core 14/pair 3/list 9/string 11/symbol 2/io 6/char 2）——`ns/本地名` 注册为与底层共享同一 `Rc<BuiltinFn>` 分派体（同 r38 别名层 parity 形态）；**R-N4 词法零改动**（`/` 已在双路径 ID 域——`str/append` 单 token 天然流过全管线，独立 `/` 除法维持）
- **io 族 fail-closed 分项注册**（22 §5.1 矩阵唯一对齐点「门控全集 ≡ 导出全集」）：未授权分项限定名不注册；R9 `gated_name` 归一化——`io/*` 限定名与门控名同表数据源，未授权引用 → E0006 携「io/print」限定形态（红线 1 双门分立可观测形态）
- **E0014 限定名不导出**（R-N3 不回落——22 §3.3）：`driver verify_qualified_refs` 编译期验证（仅查 stdlib 七面 export + 接管豁免；**Lambda 参数遮蔽**——R-N1 N3 局部胜出[开发实录：初版漏遮蔽集，测试实测发现当场修复]；诊断「不导出」而非「未绑定」——打错模块名精确指向模块面）
- **E0015 保留域违例**（22 §7）：用户模块名 `kerf-` 前缀 → 编译期拒绝（许可名单八名：preamble + 七模块标识形）
- **限定名签名派生**：`builtin_sigs` 按模块表派生（`ns/name` 与底层同签名——check/hm 对限定名同判；read-line 底层不列口径维持）

### 交付二：§3.2 六命令 + 审计 + CLI + 对账

- **788 → 806（净 +18）零回归**（集成 +17：namespace_tests 新文件[tests/v0/stage3/plan/ 首件]；单元 +1：模块表闭合守卫[47 行数/分面计数/131-119 计数锚/派生签名]）
- 四审计集 EXIT 0 ×4 维持（190）+ CLI 四路径（fib 144/macros 42/effect_stress 120/E0005 REAL_EXIT 1）+ **限定名端到端实证**（七模块正路 + E0014/E0015/E0006 限定形态负路 + 除法/符号值锚）
- 对账：matrix v0.1.0-r39 + pipeline r39（Tier 2 头 571→588）+ 六文档回写（18 v6.4[E0014/E0015 落位回填——「先查本表占位」纪律兑现]/20 v1.4/22 v1.2[§8 M1 落地状态]/12 v6.11/23 v1.2[窗 M M1 进行中]/09 v6.8）+ v0.5-roadmap 批次 M M1 行 + TD 登记册零新事件

### 交付三：r39 tar.gz 包内自举 + web E2E + rec 树（60-z/60-web）

- r39 tar.gz（§19.3 正序）+ 包内自举（全新解包构建 + 806:0:0 复跑 + 包内四审计集 + CLI 一致 + qbe 在包）
- web 面：kerf-data r39 节点 + footer v7.9 + download README r39 节 + E2E 双端 + lint 0 + git 双仓 clean
- rec 树：02_r39 新建（03 层）+ l 更新

### 里程碑：批次 M M1 ✅（限定名可见面核心）——M2 = import 注入面/别名 + B1-B3 契约 + W 弃用族 + 冲突三类 + 组合闭包（22 §8 驱动）
