...
| **其余** | 不在 Stage 0 设计范围 | LSP、类型检查器、本地码生成、优化器、FFI、并发 | P4 完全推迟 |
...
*文档结束。此 v11.0 版本完成了设计蓝图（stage0.md v5.0）精华向 sop.md (v10.0) 的同步吸收：在保留原有全部流程协议的基础上，将核心设计原则扩至二十八条（§2.2）、新增核心架构原则（§2.4）与多 crate 项目结构规则（§8.4.6），并把能力引入时机与处理程度规划（§21.7-§21.11）接入阶段推进流程的对应节点。*
