...
|---|---|---|---|
| B | **TD-004 scope-set 解析（A4 重排——绑定表基建 + VarRef scope 桥 + 双路径解析切换）** → TD-002 符号值 → 标准库最小集（列表/字符串/I/O 各 ≥8 函数 + 测试 1:3）→ Reader kerf 重写（token 化 + parse 全程 VM 运行） | 07 §3.3 B 组信号逐项 | 22-x |
| C | 类型检查器（Rust 外部 + 循环依赖缓解落地）→ 编译缓存（13 §3.1.4 三方法规格直落地）→ TD-016 收紧 + TD-013 设计批 | §21.3 条件 4 增量编译可用 | 23-x |
