...
| E | TD-004 scope-set 收口 ✅（r13，Task 32-a：展开器 fresh scope 深注入 + VarRef/SetBang/Lambda 作用域集携带 + 编译器/eval 双路径 (name, scopes ⊆) 子集匹配——500 基线零回归 + 锚点 9 测试，509:0:0）→ Expander kerf 重写（E1——scope-set 语义基座就绪，Rust 实现保留为 parity oracle）→ TD-021 hof 用户面注入（随模块系统）→ Stage 1 门审查（§7.3 审计 ≥30 case + §21.3 四条） | 07 §3.3 全部 + §21.5 全部 | 32-x（r13 起） |
