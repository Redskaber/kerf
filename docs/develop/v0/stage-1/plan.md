...
| E | TD-004 scope-set 收口 ✅（r13，Task 32-a：展开器 fresh scope 深注入 + VarRef/SetBang/Lambda 作用域集携带 + 编译器/eval 双路径 (name, scopes ⊆) 子集匹配——500 基线零回归 + 锚点 9 测试，509:0:0）→ E1-α 自举 Expander ✅（r14，Task 33-a：expander.krf 核心形式 + 九糖 + 内部 define 提升 + fresh-scope 注入 + quote 转换，VM 上运行；bootstrap_expander.rs 双向值桥；parity 19 测试——结构/Span/作用域集/param_scopes/错误消息逐字一致 + 行为面端到端可执行；**影子路径**——生产展开仍走 Rust 种子，528:0:0）→ **E1-β 宏收口 + 生产切换 + TD-021 ✅（r15，Task 34-x：expander.krf 宏系统（define-syntax/syntax-rules 全模式面：通配/字面量/省略号零-多段-复合/向量/嵌套 + 卫生 α 重命名 $hyg$N + 深度计数 500 + Span 并集代次守卫镜像（节点第 4 字段 expansion_id + retag +1——Span::merge 保守策略逐位镜像）+ 变换器注册表单表（用户宏覆盖糖名 + 卫生基名回退对两类同权））+ 生产切换（compile_front 展开段经 bootstrap_expander——读+展开两段全自举；守护测试 production_expander_is_bootstrap 独立线程活性探针）+ TD-021（kerf-prelude 模块 forms 级合并注入 + 多模块按序 declare + 主模块 visit）；parity 19→36（宏 17）+ prelude_tests 7；553:0:0）→ Stage 1 门审查 ✅（r15，Task 34-d：stage1_gate_audit_r1 50 case APPROVED——§7.3.1 配比满足（A12/B12/C10/D6）+ §7.3.2 边界 6（Span 代次守卫/require·set 代次/prelude/module 宏注册）+ §7.1.1 七类全覆盖 + 修复发现项 2（模块注册簿错误无 Span 定位/无渲染——挂 module 形式 Span + render_diagnostic）+ §21.3 四条件锚定（条件 1 生产管线自举含宏/条件 2 VM 承载/条件 3 prelude+stdlib/条件 4 缓存确定性+命中观测）** | 07 §3.3 全部 + §21.5 全部 | 32-x/33-x/34-x（r13-r15） |
