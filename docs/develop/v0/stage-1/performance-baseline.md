...
### 4.2 r25 / 42-f 复测（效应面落地成本——同会话交错对拍）

**方法**：worktree 重建 r24 二进制（a7a11f8）与本会话 r25 **交错成对**
测量（消除机器状态漂移——单次连续测量存在 +10% 级漂移，成对差稳定）；
bench 内置口径（每轮 = 自举前段 + 编译 + VM）。

| 口径 | r24（交错对拍） | r25 | Δ（成对差中位） | 判定 |
|---|---|---|---|---|
| gc_stress（尾形）每轮 | ~38.9ms | ~40.7ms | **+4.5%** | 噪声带 ✓ |
| fib(25) bench 每轮 | ~85.2ms | ~88.5ms | **+3.9%** | 噪声带 ✓ |
| gc_stress_nontail（非尾形）每轮 | ~95.1ms | ~103.5ms | **+6.2%** | **带外 1pct**（如实归因，见下） |

**归因（实验排除法）**：① GcCell 判据三模式（+Continuation）——对照
实验（临时退回两模式重建）不降反升（115 vs 103.5ms——非单点主因，
属噪声叠加；判据已恢复三模式——GC 正确性必要）；② execute 加 200 行
后的代码布局漂移（I-cache/内联边界——大函数不可逐行归因）；③ 自举
krf 体量增量（expander.krf +~90 行 / compiler.krf +~120 行——前段
编译与 VM 运行成本，每轮恒定叠加）；④ Call 路径的 Continuation 分支
位于 Bytecode guard 之后（非效应程序零新增判别——排除）。**处置**：
非热点恶化（尾形/fib 带内 ✓；非尾形带外 1pct 且为功能新增的必要
成本——效应语言面 M1-M5 全量落地）；**不做推测性微优化**（§2.3-11
先实测禁臆测——单点归因实验已做且不成立）；42-g 门审若认定超标，
对症候选 = Value 判别序调整（Continuation 挪 Builtin 后使 has_heap
三模式连续）+ execute 拆分（效应臂外移——布局隔离）。
...
| 2026-09-11 | **r25 / 42-f 效应面落地**（Effect M1-M5——ext1/两指令/continuation/GC 六来源；worktree 交错成对对拍 §4.2） | 85.2→88.5ms（+3.9%） | 38.9→40.7ms（+4.5%） | fib/gc_stress 带内 ✓；**非尾形 95.1→103.5ms（+6.2%——带外 1pct）**：归因布局漂移 + 自举 krf 体量 + GcCell 三模式（对照实验排除单点主因——§4.2）；非热点恶化，42-g 门审复核 |
