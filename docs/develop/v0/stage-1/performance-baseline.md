...
> **Date**: 2026-09-11
> **Version**: v0.3.0-perf-baseline-r16
> **Status**: Active
...
| **当前回归/观察项（2 项）** | ① **TD-023**：gc_stress +29~46%（回归，批次 I2）；② 自举前段 69~388× 比值（**非回归**——r15 切换的既定架构成本 + 随复杂度放大特性登记为 Stage 2 排程约束，见 §5 结论 3） |
...
| 热点/候选 | 状态 | 证据/处置 |
|---|---|---|
| gc_stress 深递归根扫描（per-cycle HashSet + Value 宽度） | **回归登记** | TD-023（P2）→ 批次 I2；本基线 §4 |
