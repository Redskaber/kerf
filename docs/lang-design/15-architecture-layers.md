...
> **Date**: 2026-09-10（v6.0：§5 增补 §5.3 表面/内部语法分离与四层正交的交汇注记——next3.md 第七轮吸收；v5.5：新增 §5 四层正交对照与 IR 层级演进）
> **Version**: v6.1（v6.1 完整性审查联动：预留层 4→14 项注记 + 查询双预留互链）
> **Status**: Active
...
上图「接口预留」子图仅示既有四项（M-P）；v6.1 完整性审查后预留层扩至 14 项（新增 LSP 查询/调试信息/FFI 边界/增量查询/服务化/多后端/包管理/AI 辅助等 10 项——完整清单与依赖落位见 [13-能力矩阵 §3.4](./13-capability-matrix.md) 矩阵与 [§3.5.3 全景图](./13-capability-matrix.md)）；「完全推迟」子图相应缩减（LSP 与 FFI 移出）。
...
v6.1 注记：该架构的 Stage 0 接口预留（`QuerySystem`/`Query` trait，P0）见 [13-能力矩阵 §3.3.5](./13-capability-matrix.md)——与 [13-能力矩阵 §3.1.4](./13-capability-matrix.md) 编译缓存构成「缓存（数据面）+ 查询（架构面）」双预留；Stage 1+ 的实现应直接落地这两个 trait，而非另起接口。
...
### 5.3 表面/内部语法分离：四层正交的第五个正交轴（v6.0 吸收）

next3.md 第七轮确立的**表面/内部语法严格分离（原则 31）**与 §5.1 四层正交同构——可视为正交性的第五轴：表面语法（皮肤，可替换——S 表达式/中缀/DSL）经 Reader 桥接到内部语法（骨架，不变——CoreExpr 私有 ADT），Reader 是唯一桥接点：

| 分离轴 | 可变侧（皮肤） | 不变侧（骨架） | 桥接点 |
|--------|--------------|--------------|--------|
| 表面/内部语法 | 表面语法（Stage 0 S-expr → Stage 2 目标语法 → Stage 2+ 多语法） | CoreExpr 私有 ADT（核心冻结） | kerf-reader（[02 §1](./02-syntax-model.md)） |
| 语法/类型（§5.1 Layer 1-2） | 类型推导策略（保守 → 完整） | CoreExpr 无类型标注 | kerf-compiler::typecheck |
| 语法/效应（§5.1 Layer 1-3） | 效应处理策略（内部一次性逃逸 → 语言级） | CoreExpr 无效应字段 | kerf-driver::effects |
| 语法/能力（§5.1 Layer 1-4） | 能力策略（R9 验证 → 线性类型） | CoreExpr 无能力字段 | kerf-driver::capability |

**结论**：r8/r9 的 crate 边界（reader/syntax/expander/core 与 compiler/driver 的组合根分工）同时满足五个正交轴——表面语法的替换（Stage 2 目标语法）不影响任何一层的内部结构。设计规范锚点：[01-核心原语 §8.4](./01-core-forms.md)；分阶段策略：[12-路线图 §2.4.1](./12-roadmap.md)。
