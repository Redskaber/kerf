...
基础设施层的三个组件（Span、诊断、相位分离）是**横切关注点**：它们不处于编译管线的主数据流上，却被管线每个阶段消费。本章论述其架构约束；数据结构与能力模型的规范定义分别在 [02-语法模型 §2/§3/§4/§5](./02-syntax-model.md)（Span、诊断）与 [03-宏系统 §1/§3](./03-macro-system.md)（相位分离），二者构成"定义 vs 架构"的分离视图。
...
9 个核心原语的完整小步操作语义（归约规则），以及编译正确性定理：对任意源程序 `p`，如果 `p →* v`，那么 `compile(p) →* v`。已拆分至 [06-操作语义](./06-operational-semantics.md)。
