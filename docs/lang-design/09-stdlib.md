...
> **Version**: v6.10（**r41 / 62-a 深审 D10 注记**：E0020 保留字绑定禁令——25 关键字全域禁作绑定名[Stx + CoreExpr 双层]；词法语法语义二义性面全清[22 §11 D18]；v6.9：r40）（r40 / 61-b 批次 M M2：**W1001 旧名弃用警告族 v0.6 弃用期启动**——27 旧名引用 → `[W1001]` stderr 非阻断警告（每名去重 + 现代名指引 + preamble 结构性豁免[引导语料 20 §6.5 纪律]）；W1002 遮蔽注入名警告（22 §3.2 R-N2 第二行）；B1/B2 限定名新契约 miss→nil——旧名/扁平名契约不动[20 §4 迁移不变量]；v1.0 前夜移除轮清算）（**r39 / 60-a 批次 M M1**：§2 增限定名可见面注记[47 限定名 + E0014/E0015]；v6.7：r38 批次 L 别名层实施；v6.6：r34 十一域口径；v6.5：r33；v6.4：r32 命名规范层首增）
> **Status**: Active
