...
> **Date**: 2026-09-09（v5.1 增补：处理程度标注 + 实现落点对账）
> **Version**: v5.1
> **Status**: Active
...
> 该清单为 Stage 0 的**最小骨架**：仅保证自举与测试所需（同 [12-路线图 §3.3 里程碑验证清单](./12-roadmap.md) 的要求）；标准库的完整生长（列表操作、字符串处理、基本 I/O 的库化）是 Stage 1 → Stage 2 的阶段切换信号之一（见 [07-自举策略 §3.3](./07-bootstrap-strategy.md)）。**Stage 0 实现落点对账**：driver 实际注册 20 项内置函数（worklog Task 4-c，含卫生回退解析 `$hyg$` 后缀剥离）——较本表最小集扩充的类型谓词与串接函数均属「同一注册机制的加法」，未引入新核心形式；完整清单以 `kerf-driver` 实现为准（δ 函数表，[06-操作语义 §2 A4 规则](./06-operational-semantics.md) 的 δ 语义载体）。
