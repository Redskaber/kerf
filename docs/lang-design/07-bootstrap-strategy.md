...
> **Date**: 2026-09-15（**v6.3：r38 / 59-a 批次 L 语料纪律注记**：§3.2 B3 交付注记后增「引导语料双注册注记」——引导三件 + 序章语料继续以旧名工作（v0.5 别名层双注册保证——[20 §6.5](../../lang-design/20-surface-conventions.md)），新语料一律现代名书写（09 §4 纪律）；私有 helper 机会主义迁移维持）；2026-09-10（v5.2：补四项接口预留契约位置指引（#26））
> **Version**: v6.3
> **Status**: Active
...
> Stage 2 决策点）。
>
> **引导语料双注册注记（r38 / 批次 L，v6.3）**：v0.5 别名层交付后（[20-表面规范](../../lang-design/20-surface-conventions.md) §6.4/§7——注册面 57→84），
> 引导三件（reader.krf/expander.krf/compiler.krf）+ 序章（preamble.krf）
> **继续以旧名工作**（双注册保证——旧名存量面零破坏，移除轮 Stage 3 才触）；
> **新语料一律现代名书写**（[09-stdlib §4](../../lang-design/09-stdlib.md) 纪律——bootstrap 新增代码/examples 新件/测试新 case）；私有 helper（`err?`/
> `is-keyword?`/`num->str` 等自铸名）机会主义迁移维持（[20 §6.5](../../lang-design/20-surface-conventions.md)——零用户影响面，各批次触碰对应文件时顺带收编）。
