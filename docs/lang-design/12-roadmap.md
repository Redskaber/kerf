...
| 元循环求值器 | 实现中（P0） | ~~重写升级~~ → **保持（Rust 参考）**（v6.2 改写：Stage 1 实际交付 = 前端两段自举（Reader r6 + Expander r15），**eval 重写滑入 Stage 2**——07 §3.2 混合期构成与交付为准；参考路径角色保持至被编译器替换） | **重写升级（eval 自举迁移）** + 被编译器替换（I1 范围） | 移除 |
