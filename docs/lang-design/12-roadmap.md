...
> **Date**: 2026-09-15（**v6.9：r37 / K3 收尾交付——Stage 2 全收口**：本文 §2.5.1（附加）类型检查器行 K3 终态注记（默认期评估闭环——HM 旗标期维持 + 默认期切换窗口归 23 §2.2 治理触发表 Stage 3 承载）+ §2.10 v0.5-roadmap 预锚兑现（v0.2.0 Stage 2 行收口——批次 J/K 全链终态 + Stage 3 GO + 批次 L/M 入场序列）；批次 K 全闭环（K1 APPROVED + K2 GO + K3 收尾）→ Stage 2 全收口）；2026-09-14（**v6.8：r34 / 55-a 设计缺陷深度审计收敛轮**——设计栈治理层补齐（[23-演进治理](./23-evolution-governance.md) v1.0——十二审计轴 + 演进六窗触发表 + 准入总表 + 收敛证明）：本文 §2.10 的批次 L/M 时机由 23 §2.2 触发表统合承载（入口信号/出口条件单源化）；21 v1.1（三轴坐标系 + 组合闭包 + 编译期权威 + 十一域）+ 22 v1.1（矩阵两行）+ 原则 35 判据先于先例）；2026-09-12（**v6.7：r33 / 54-a 能力架构深度设计轮**——§2.10 批次 M 机制设计输入补齐（[22-命名空间设计](./22-namespace-design.md)——五层 N0-N4/解析 R-N1~N8/权限矩阵/实施对账表 §8）+ 批次 M 行精确化口径（[21 §4.3](./21-capability-architecture.md) 不变量 A）；**v6.6：r32 / 53-a 表面现代化审查轮**——新增 §2.10 表面命名现代化迁移（批次 L/M/移除轮版本口径 + TD-027 登记 + v0.5-roadmap 预锚）；v6.5：批次 J 收尾终态复核（r31/52-a——plan §5b 48-e J4 排程注 4 兑现）——§2.5.1 十一行注记批次 J 终态对账三处**：Effect Handlers 行 48-b 静态收敛面 ✅ 兑现（r28）+ 字节码 VM+GC 行 FFI 所有权 GC 边界注记（r30 Heap Φ 簿——§21.3 条件 4 兑现）+ 类型检查器行批次 J 兑现位终态（默认期评估移交批次 K 终门审承载）**；（v6.4：批次 J 执行轮回写（r29/50-a）——§2.5.1 类型检查器行兑现：HM 旗标期切换落地（D8 阶段 2）+ 自举内迁裁定留 Rust（INC8 三段口径 + INC3 静态分析面不在自举关键路径）；（v6.3：批次 J 规划轮回写（r27/48-a）——§2.5.1 Stage 2 列九行处置注记：已兑并行四（闭包/VM+GC/Effect M1-M5 补记/类型检查器排位批次 J）+ 多阶段与缓存增量两行 DEFER Stage 3（成熟度重评/收益边际）+ net/process 触发式 + 宏完整化改判 Stage 3——依据 [stage-2/plan §5d 处置表](../../develop/v0/stage-2/plan.md)）；（v6.2：批次 F 深审回写——§2.5.1 两行矩阵按交付实况改写（元循环求值器 Stage 1 保持/宏系统 E1-β 收口）+ §2.5.1 后注 Stage 1 帧标注；2026-09-10（v6.0：§2.5.1 增补「原语演进评估清单注记」——01 §7.3/§8.3 所指登记落位（v5.5 吸收时悬空引用修复）；v5.4：r8 批次 D 交付注记）)
> **Version**: v6.13（**r41 / 62-a+b 批次 M M3 收口 + 语言形式深审**：§2.10 批次 M 行 M3 ✅[E0020 + M3 门审 43 case APPROVED——窗 M 出口条件达成]；v6.12：r40）（**r40 / 61-a+b 批次 M 次件 M2**：§2.10 批次 M 行 M2 注记[import 注入面/别名 + 组合闭包 P1 缺口修复 + B1/B2 契约 + W1001/W1002 + E0013/E0016-E0019 + 825:0:0——深审 D1-D9 见 22 §10]；v6.11：r39 M1；v6.10：r38 批次 L；v6.9：r37 / K3 收尾；v6.8：r34；v6.7：r33；v6.6：r32 §2.10 首增）
> **Status**: Active
...
- **Phase 4 接口预留**：冻结预留接口的类型签名——v6.0 前为 4 项（Effect/多阶段/能力 I/O/编译缓存）；v6.1 完整性审查后扩至 14 项（+LSP 查询/调试信息/FFI 边界/增量查询/服务化/多后端/包管理/AI 辅助等，[13-能力矩阵 §3.4](./13-capability-matrix.md)）。依赖前面的类型系统定型（接口形状要引用 Value、CodeValue、Span 等已定型类型）
...
| 结构化 CodeValue | 实现中（P0） | 增强（宏集成） | 升级（多阶段支持）**——升级面随多阶段同窗 DEFER Stage 3（v6.3/r27）；Stage 2 实际增量 = continuation 值（r25）+ ForeignBox 装箱（r24）已兑现** | 优化（JIT 预备） |
| 元循环求值器 | 实现中（P0） | ~~重写升级~~ → **保持（Rust 参考）**（v6.2 改写：Stage 1 实际交付 = 前端两段自举（Reader r6 + Expander r15），eval 重写滑入 Stage 2——07 §3.2 混合期构成与交付为准；参考路径角色保持至被编译器替换） | ~~重写升级（eval 自举迁移）~~ → **不迁移（r23/42-d 终态裁定 INC7）** + **被编译器替换 ✅ 交付（I1 范围）**：自举编译器（compiler.krf，VM 上运行）+ VM 执行 = 唯一生产路径（CompilerKind 生产切换）；eval 生产面退役（CLI eval 子命令移除 + driver eval_source 删除——TD-017/TD-009 域随路径注销）；kerf-vm eval.rs 存档为 Rust 参考实现（语义 oracle 消费面保留）；T1 双路径互查新口径 = 生产链 vs 种子链（run_source_seed） | 移除（eval.rs 存档面随 Stage 3+ 清理） |
| 基础闭包 | 实现中（P0） | 保持 | 升级（Effects 引入后语义协调）**✅ 已兑现（v6.3/r27 注记：r24 TD-010 闭包 Foreign 装箱 + 追踪器协议；r25 效应帧与闭包帧 GC 六来源并存协调）** | Effects 生态 |
| Effect Handlers | 预留定义（P3） | 做实引入（编译器内部）✅ | **设计做实（r18 注记）**：语言级设计冻结于 [effect-language-design.md](../develop/v0/stage-2/effect-language-design.md)（40-b E2 GO-DESIGN 兑现——12 裁定 + R10/R11 归约 + E4 三要素 + D12 实现窗口 = 批次 I 后段）；实现随 I 后段落地（M1-M5 迁移路径）**；M1-M5 落地 ✅（r25/42-f）——静态收敛面 ✅ 批次 J 48-b 兑现（r28/49-a：Perform 效应值 + Handle 两体入 R1-R8/HM 双检出域子表达式遍历，6 违例 case 双面检出 + 零误报对照——深审 D3/D8 覆盖缺口闭环；v6.5/r31 终态复核注记）** | 异步生态 |
| 多阶段编程 | 预留定义（P3） | 预留保持（类型扩展） | ~~做实引入（实验）~~ → **DEFER Stage 3（v6.3/r27 成熟度重评）**：§21.3 四条件不含 + 研究前沿档（§2.1.1）+ Stage 2 零消费场景——矩阵自述「可依据当时成熟度重新评估」结构启用 | 编译期优化 |
| 能力模型 I/O | 预留定义（P2，v5.2 修订） | 做实引入（基础传递）**✅ r8 做实（require/R9-E0006/IoGrant 令牌门控）** | 做实引入（完整模型）**——Stage 2 交付面 = 模型层完整（v6.3/r27：骨架 r19 + 族无关形状 M2 r25 + 手术面三点加法 D11 收敛）；io 族运行面做实 r8；net/process = Stage 3 触发式（窗口核对四依据——[stage-2/plan §5d net 注](../../develop/v0/stage-2/plan.md)）** | 安全沙箱 |
| Span 系统 | 实现中（P0） | 保持 | 保持 | 保持（永续增强） |
| 诊断框架 | 实现中（P0） | 保持 | 保持 | IDE 集成 |
| 宏系统 | 实现中（P1） | **自举重写 + 宏收口（E1-β r15）✅**（v6.2 改写：syntax-rules 全模式面（单层边界）+ 卫生 α + 深度 10_000（r18/TD-007 全解除——v6.3/r36 回写对齐 03 v6.3）+ 代次守卫，expander.krf 生产路径 + parity 36 测试；~~增强（调试工具）~~——03 §2 显式推迟 Stage 2+，此前矩阵行失真） | 完整化（syntax-parse/嵌套省略号 TD-005 + 调试工具）**——Stage 2 交付面 = E1-β 收口子集（在用宏面零缺口——审计集 53 case 宏面全过）；TD-005/调试工具改判 Stage 3 宏增强窗口（v6.3/r27——登记册同步）** | DSL 生态 |
| 编译缓存 | 预留定义（P2） | 做实引入（查询式）**✅ r7 做实（内存内容寻址 + 三方法 + 管线复用）** | ~~深化（增量编译）~~ → **DEFER Stage 3（v6.3/r27）**：Stage 2 交付面 = 内容寻址 + CompilerKind 分桶（r23 B11）；单文件编译模型增量收益边际 + fixpoint 确定性已达成（§21.3 条件 2 不依赖增量） | 并行查询 |
| 字节码 VM + GC | 实现中（P0） | 保持 | 升级（本地码后端并存）**✅ 已兑现 PoC 口径（v6.3/r27 注记：r17 QBE fib native + r26 门审 P04 复验；native 效应化 = TD-024 Stage 3 后端演进）+ FFI 所有权 GC 边界 ✅（v6.5/r31 注记：r30/51-a——Heap Φ 计数簿（supp(Φ) 根集第五来源计数化）+ ForeignBox 装箱/线性令牌不参与 GC 可达性；§21.3 条件 4 兑现——[13-能力矩阵 §3.3.4](./13-capability-matrix.md) 实现锚）** | 分代 GC / JIT |
| **（附加）类型检查器** | 完全推迟（P4） | **✅ r7 提前引入（保守静态 R1-R8 + 多错误 E0005——循环依赖风险缓解「外部 Rust 实现」路径落地）** | 迁移评估（自举内迁）**✅ r29 兑现（v6.4：HM 旗标期切换落地——`kerf check` 判定面 = hm_check_program（D8 阶段 2，710:0:0 + 契约重定义 §2.3）；自举内迁裁定 = 留 Rust——~80% 口径按 INC8 三段达成（读+展开+编译 100% kerf），类型检查器属静态分析面不在自举关键路径（INC3 实证：front_from_core 无 check），归 07 §3.4 Rust 保留面；默认期（R1-R8 内化）= Stage 2 末评估——移交批次 K 终门审承载（§21.5 九信号全核对——v6.5/r31 终态复核注记）；**r37 / K3 闭环（终态）：K1 P07 九信号核对 + K2 深审 GO + K3 收尾——默认期评估结论 = HM 旗标期维持（判定面 = hm_check_program；R1-R8 退为回归基线断言双面检出纪律保留）；默认期切换窗口归 23 §2.2 治理触发表 Stage 3 承载（触发式非时间驱动——时间治理四红线之一）**）** | 类型生态 |
...
### 2.9 接口预留成本核算（v6.1 新增，原 §21.9）

[13-能力矩阵 §3.5](./13-capability-matrix.md) 的完整性审查在本文 §2.8 净影响评估之上追加**接口预留专项**成本维度：

#### 2.9.1 预留成本与净影响的关系（原 §21.9.1）
...
#### 2.9.2 处理程度标度的接口维度应用（原 §21.9.2）

本文 §2.1 的五级标度（P0-P4）原本度量**能力**的处理程度；v6.1 将其延伸到**接口**维度：接口预留 = 在 P3「仅类型形状」或 P2「类型 + 行为规格」级别冻结签名，实现推迟到对应 Stage（[13-能力矩阵 §3.4](./13-capability-matrix.md) 矩阵）。二者的叠加规则：**能力引入时机（本文 §2.5 演进矩阵）决定接口何时被实现；接口预留时机（[13-能力矩阵 §3.5.1](./13-capability-matrix.md)）决定数据结构位置何时被冻结**——前者可以推迟，后者必须在 Stage 0（P0 项）或 Stage 1（P2 项）完成，否则破坏渐进替换原则（[17-设计原则 §1](./17-principles.md) 第 28 条）。

### 2.10 表面命名现代化迁移（v6.6 新增——r32 / 53-a 审查轮；v6.7 r33 注记：批次 M 机制设计输入补齐 [22-命名空间设计](./22-namespace-design.md)；v6.8 r34 注记：批次 L/M/E5 的**入口信号与出口条件**由 [23-演进治理 §2.2 触发判据表](./23-evolution-governance.md) 单源承载——本节为迁移内容登记，时机治理归 23）

> 驱动源：用户审查指令（r32）——「实现端不要停留在 Lisp 家族 1970 年代表达」；实测见 [20-表面规范 §1](./20-surface-conventions.md)（57 内置名 + 扁平命名空间 + 行为模式遗留——此前无现代化 owner/TD 登记）。本节登记其批次位置与版本口径（规范本体 + 57 项映射表 + 行为契约 B1-B5 = [20-表面规范](./20-surface-conventions.md)）。

| 批次 | 版本 | 内容 | 与既有批次关系 |
|---|---|---|---|
| 批次 L（别名层） | v0.5 | 27 现代名双注册（旧名不动——行为 parity 锚定）+ `BUILTIN_SIGS`/门控表三方同步 + parity 测试组 + 新语料现代名纪律 ——**✅ r38 交付**（[20 §6.4/§7](./20-surface-conventions.md)：84 注册 + 83 签名 + 门控零变更实测 + 27 parity case + 三方守卫 ×2；§3.2 788:0:0 零回归——23 §2.2 窗 L 出口条件三全过） | 独立于批次 K（Stage 2 终批）——v0.5 首批；纯增量零破坏（E5 裁定适用域划分——[20 §6.2](./20-surface-conventions.md)） |
| 批次 M（命名空间层） | v0.6 | Reader `/` 限定名 + `kerf/<模块>` 标准库模块树 + B1-B3 现代契约 + 旧名弃用警告（W 级诊断族）——**M1 ✅ r39**（限定名可见面：七模块 47 限定名 + E0014 不回落/E0015 保留域 + io 双门分立 + 签名派生——806:0:0）+ **M2 ✅ r40**（import 注入面/别名[as contextual + 编译期归一] + 组合闭包[21 §4.4 落地 + **D4 P1 缺口修复**：from_core 顶层口径] + B1/B2 契约[限定名 miss→nil 独立分派体；B3 纯测试锚] + W1001/W1002 弃用与遮蔽族 + E0013/E0016/E0017/E0018/E0019 五码 + namespace_tests 36 case——**825:0:0 净 +19**；深审 D1-D9 裁定 [22 §10](./22-namespace-design.md)；M3 ✅ r41（**收口门审 + 语言形式深审**：E0020 保留字绑定禁令[D10 P1 文档-代码冲突当场修复——22 §11 八焦点裁定] + **stage3_gate_audit_m3 43 case APPROVED**[七类全覆盖 + 三修复边界 ≥5 + E2E 全导入路径三通道]——**837:0:0 净 +12，窗 M 出口条件达成[23 §2.2]**）） | 依赖批次 L（✅ r38）；**机制设计输入 = [22-命名空间设计](./22-namespace-design.md)（r33/54-a 补齐——五层 N0-N4 + 解析 R-N1~N8 + 权限矩阵 + §8 实施对账表；r39 M1 + r40 M2 落地状态注记 v1.3）**；能力-命名空间对齐（`kerf/io` ↔ `(require io …)`——精确化口径 [21 §4.3](./21-capability-architecture.md)） |
| 移除轮 | Stage 3 / v1.0 前夜 | 旧名移除 + **E5 关键字切换同窗**（[01 §7.2/§8.3](./01-core-forms.md)——`lambda→fn`/`set!→set` 候选） | 一次性清算「1970 表面」（[20 §6.3](./20-surface-conventions.md)）——**✅ S1 表面腿 r42**（E5 窗开启[入口四信号全过——e5-plan] + 旧名 27 件退役 E0021 + W1003 + 828:0:0；S2 关键字腿 = E5 后续腿） |

**与 §2.5.1 演进矩阵的关系**：表面命名不是能力模型（不入矩阵行）——它是**横切规范**（同 Span/诊断的横切关注点定位）；矩阵中受影响的行仅「标准库」（Stage 2 库化生长）——生长面按新规范书写。**TD 登记**：TD-027（实施跟踪 owner）；设计 owner = 20-表面规范。**v0.5-roadmap 预锚**（批次 K 48-h/K3 交付项）：v0.5 首批 = 批次 L（本节为 K3 的 v0.5-roadmap 提供登记锚）。
