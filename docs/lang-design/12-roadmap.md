...
> **Version**: v6.9（r37 / K3 收尾：类型检查器行 K3 终态注记 + §2.10 v0.5-roadmap v0.2.0 预锚兑现——Stage 2 全收口；v6.8：r34 / 55-a：23-演进治理指针 + 时机治理单源化注记；v6.7：r33；v6.6：r32 §2.10 首增）
> **Status**: Active
...
| 批次 | 版本 | 内容 | 与既有批次关系 |
|---|---|---|---|
| 批次 L（别名层） | v0.5 | 27 现代名双注册（旧名不动——行为 parity 锚定）+ `BUILTIN_SIGS`/门控表三方同步 + parity 测试组 + 新语料现代名纪律 | 独立于批次 K（Stage 2 终批）——v0.5 首批；纯增量零破坏（E5 裁定适用域划分——[20 §6.2](./20-surface-conventions.md)） |
