...
> **Version**: v6.10（**r38 / 59-a 批次 L 实施兑现**：§2.10 批次 L 行 ✅ r38 交付注记[27 别名双注册 + 788:0:0 + 27 parity 全绿]；v6.9：r37 / K3 收尾：类型检查器行 K3 终态注记 + §2.10 v0.5-roadmap v0.2.0 预锚兑现——Stage 2 全收口；v6.8：r34 / 55-a：23-演进治理指针 + 时机治理单源化注记；v6.7：r33；v6.6：r32 §2.10 首增）
> **Status**: Active
...
| 批次 M（命名空间层） | v0.6 | Reader `/` 限定名 + `kerf/<模块>` 标准库模块树 + B1-B3 现代契约 + 旧名弃用警告（W 级诊断族） | 依赖批次 L；**机制设计输入 = [22-命名空间设计](./22-namespace-design.md)（r33/54-a 补齐——五层 N0-N4 + 解析 R-N1~N8 + 权限矩阵 + §8 实施对账表）**；能力-命名空间对齐（`kerf/io` ↔ `(require io …)`——精确化口径 [21 §4.3](./21-capability-architecture.md)） |
