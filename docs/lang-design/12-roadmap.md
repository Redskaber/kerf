...
> **Date**: 2026-09-10（v5.4：r8 批次 D——Effect 编译器内部做实（一次性逃逸层）与能力模型 I/O 基础传递做实（require/R9-E0006/令牌门控）交付注记（§2.4.3/§2.4.5/§2.5.1 三处）；v5.3：r7 批次 C 交付注记——Stage 1 列「引入类型检查器」与「编译缓存做实」两项落地；v5.2：GC 验收双轨口径（#2）+ 能力模型 I/O P3→P2（#21）+ benchmarks 锚点改指 CLI bench（#22））
> **Version**: v5.4
> **Status**: Active
