...
> **Version**: v6.12（**r40 / 61-a+b 批次 M 次件 M2**：§2.10 批次 M 行 M2 注记[import 注入面/别名 + 组合闭包 P1 缺口修复 + B1/B2 契约 + W1001/W1002 + E0013/E0016-E0019 + 825:0:0——深审 D1-D9 见 22 §10]；v6.11：r39 M1；v6.10：r38 批次 L；v6.9：r37 / K3 收尾；v6.8：r34；v6.7：r33；v6.6：r32 §2.10 首增）
> **Status**: Active
...
| 批次 M（命名空间层） | v0.6 | Reader `/` 限定名 + `kerf/<模块>` 标准库模块树 + B1-B3 现代契约 + 旧名弃用警告（W 级诊断族）——**M1 ✅ r39**（限定名可见面：七模块 47 限定名 + E0014 不回落/E0015 保留域 + io 双门分立 + 签名派生——806:0:0）+ **M2 ✅ r40**（import 注入面/别名[as contextual + 编译期归一] + 组合闭包[21 §4.4 落地 + **D4 P1 缺口修复**：from_core 顶层口径] + B1/B2 契约[限定名 miss→nil 独立分派体；B3 纯测试锚] + W1001/W1002 弃用与遮蔽族 + E0013/E0016/E0017/E0018/E0019 五码 + namespace_tests 36 case——**825:0:0 净 +19**；深审 D1-D9 裁定 [22 §10](./22-namespace-design.md)；M3 = 收口门审[§7.3 ≥30 case]） | 依赖批次 L（✅ r38）；**机制设计输入 = [22-命名空间设计](./22-namespace-design.md)（r33/54-a 补齐——五层 N0-N4 + 解析 R-N1~N8 + 权限矩阵 + §8 实施对账表；r39 M1 + r40 M2 落地状态注记 v1.3）**；能力-命名空间对齐（`kerf/io` ↔ `(require io …)`——精确化口径 [21 §4.3](./21-capability-architecture.md)） |
