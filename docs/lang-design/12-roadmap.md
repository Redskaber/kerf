...
> **Version**: v6.11（**r39 / 60-a 批次 M 首件 M1**：§2.10 批次 M 行 M1 注记[限定名可见面 47 名 + E0014/E0015 + 806:0:0]；v6.10：r38 批次 L；v6.9：r37 / K3 收尾；v6.8：r34；v6.7：r33；v6.6：r32 §2.10 首增）
> **Status**: Active
...
| 批次 M（命名空间层） | v0.6 | Reader `/` 限定名 + `kerf/<模块>` 标准库模块树 + B1-B3 现代契约 + 旧名弃用警告（W 级诊断族）——**M1 ✅ r39**（限定名可见面：七模块 47 限定名 + E0014 不回落/E0015 保留域 + io 双门分立 + 签名派生——806:0:0；M2 = import 注入面/别名 + B1-B3 + W 族；M3 = 收口门审） | 依赖批次 L（✅ r38）；**机制设计输入 = [22-命名空间设计](./22-namespace-design.md)（r33/54-a 补齐——五层 N0-N4 + 解析 R-N1~N8 + 权限矩阵 + §8 实施对账表；r39 M1 落地状态注记 v1.2）**；能力-命名空间对齐（`kerf/io` ↔ `(require io …)`——精确化口径 [21 §4.3](./21-capability-architecture.md)） |
