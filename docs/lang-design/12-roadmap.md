...
> **Date**: 2026-09-11（v6.2：批次 F 深审回写——§2.5.1 两行矩阵按交付实况改写（元循环求值器 Stage 1 保持/宏系统 E1-β 收口）+ §2.5.1 后注 Stage 1 帧标注；2026-09-10（v6.0：§2.5.1 增补「原语演进评估清单注记」——01 §7.3/§8.3 所指登记落位（v5.5 吸收时悬空引用修复）；v5.4：r8 批次 D 交付注记）)
> **Version**: v6.2
> **Status**: Active
...
| Effect Handlers | 预留定义（P3） | 做实引入（编译器内部）✅ | **设计做实（r18 注记）**：语言级设计冻结于 [effect-language-design.md](../develop/v0/stage-2/effect-language-design.md)（40-b E2 GO-DESIGN 兑现——12 裁定 + R10/R11 归约 + E4 三要素 + D12 实现窗口 = 批次 I 后段）；实现随 I 后段落地（M1-M5 迁移路径） | 异步生态 |
...
| 宏系统 | 实现中（P1） | **自举重写 + 宏收口（E1-β r15）✅**（v6.2 改写：syntax-rules 全模式面（单层边界）+ 卫生 α + 深度 500 + 代次守卫，expander.krf 生产路径 + parity 36 测试；~~增强（调试工具）~~——03 §2 显式推迟 Stage 2+，此前矩阵行失真） | 完整化（syntax-parse/嵌套省略号 TD-005 + 调试工具） | DSL 生态 |
