...
| 宏系统 | 实现中（P1） | **自举重写 + 宏收口（E1-β r15）✅**（v6.2 改写：syntax-rules 全模式面（单层边界）+ 卫生 α + 深度 500 + 代次守卫，expander.krf 生产路径 + parity 36 测试；~~增强（调试工具）~~——03 §2 显式推迟 Stage 2+，此前矩阵行失真） | 完整化（syntax-parse/嵌套省略号 TD-005 + 调试工具）**——Stage 2 交付面 = E1-β 收口子集（在用宏面零缺口——审计集 53 case 宏面全过）；TD-005/调试工具改判 Stage 3 宏增强窗口（v6.3/r27——登记册同步）** | DSL 生态 |
