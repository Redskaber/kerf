...
> **Date**: 2026-09-11（v6.2：批次 F 深审回写——§2 补 CLI 11 子命令表面清单（B4 灰区收口））
> **Version**: v6.2（v6.1 完整性审查：LSP 接口预留重分类注记——实现推迟、数据结构位置 Stage 0 冻结）
> **Status**: Active
...
**CLI 表面清单（v6.2 补写——B4 设计灰区收口：实现即事实，11 个子命令此前无文档枚举）**：根 CLI（src/main.rs）实际暴露 **11 个子命令**——`run`（编译+执行）/ `eval`（eval 参考路径）/ `check`（静态检查——r7 类型检查器消费面，多错误全量收集）/ `test`（用例运行器——r8 批次 D 交付，见 [11-测试 §5](./11-testing.md)）/ `tokens` / `stx`（Reader 两层 dump——四层调试的第 1 层）/ `core` / `ir` / `bc` / `code`（Expander/Compiler 四层 dump——第 2 层，经 driver 转发不越界 §11）/ `bench`（性能基准——本文 §3 的 CLI 载体）。第 3/4 层（内省 API/交互式调试器）预留未实现（属 13 §3.3 LSP/DebugInfoGenerator 做实范围——Stage 2+）。
