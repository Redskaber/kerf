...
> **Version**: 11.2（测试入口精要 + lang-design v5.5 同步）
...
- [§2 核心原则（含二十八条设计原则 + 架构原则 + 每轮执行原则 + 决策细则）](#2-核心原则)
- [§3 环境与工具管理](#3-环境与工具管理)
...
> **来源**：最小自举系统级编程语言设计蓝图（stage0.md v5.0，核心二十八条原则）。第 1-20 条自 v2.0 沿用；第 21-25 条为 v3.0 引入的“2026 现代方案”原则；第 26-28 条为 v5.0 引入的进程裁决原则。
...
以下 11 条原则按优先级排列，冲突时上面的优先。它们是二十八条核心原则（§2.2）的工程执行细则：
