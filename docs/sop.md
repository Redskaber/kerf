...
> **Version**: 11.3（设计原则 29-31 + 蓝图存档登记 + lang-design v6.0 同步）
...
> **来源**：最小自举系统级编程语言设计蓝图（stage0.md v6.0，核心三十一条原则；蓝图全文存档于 `docs/stage0.md`，目录级拆分见 `docs/lang-design/`）。第 1-20 条自 v2.0 沿用；第 21-25 条为 v3.0 引入的“2026 现代方案”原则；第 26-28 条为 v5.0 引入的进程裁决原则；第 29-31 条为 v6.0 引入的内部语法与语法分离原则（next3 讨论终轮——见 lang-design/17-principles.md §1.1）。
...
| `docs/stage0.md` | 上游设计蓝图查询（合并全文与演进记录档案） | 设计蓝图存档（v6.0 原语批判演进版——目录级拆分权威为 docs/lang-design/） |
...
├── stage0.md            # 上游设计蓝图存档（v6.0——合并全文 + 演进记录档案，§8.4.5 查询表）
