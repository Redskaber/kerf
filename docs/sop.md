...
├── tests/                    # 【跨 crate】根项目集成测试（§9.1 的 v0/stage-N 树）
...
*文档结束。此 v11.1 版本完成 r7/r8 实施进度向 §21.9 演进矩阵的现状对账（编译缓存/类型检查器/Effect 编译器内部/能力 I/O 基础传递四项做实状态注记 + 类型检查器提前引入偏差登记，§8.4.5 规则 2 文档随代码）；v11.0 版本完成了设计蓝图（stage0.md v5.0）精华向 sop.md (v10.0) 的同步吸收：在保留原有全部流程协议的基础上，将核心设计原则扩至二十八条（§2.2）、新增核心架构原则（§2.4）与多 crate 项目结构规则（§8.4.6），并把能力引入时机与处理程度规划（§21.7-§21.11）接入阶段推进流程的对应节点。*
